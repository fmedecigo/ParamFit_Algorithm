%% Parametric Fitting and Diatom Classification
% This script performs nonlinear least squares fitting on diatom outlines 
% using a parametric model and classifies them based on the best fit.
%
% Steps:
%   1. Read diatom outline data from an Excel file.
...
%% Load seed diatom data
filePath = 'C:\contornosdiatomeas.xlsx';  
...
x_in=A(:,11); %% batch 2:(1) 000305B Cyclotella radiosa (Grunow) Lemmermann
y_in=A(:,12);

% Uncomment for other diatom types:

% x_in=A(:,5); %% Batch 1: 000073B, Navicula menisculus Schumann
% y_in=A(:,6);

% x_in=A(:,9); %% batch 1: (2) 000125B Sellaphora bacillum (Ehrenberg) D.G. Mann
% y_in=A(:,10);
 
% x_in=A(:,25); % batch 5: 000578BB Gomphonema olivaceum var. minutissimum Hustedt
% y_in=A(:,26);
